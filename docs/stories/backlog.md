...
| E04 Dashboard Reports | CSV export, date filtering, known/unknown analytics breakdown | implemented |

## Recommended Next Stories

| Candidate | Why it matters | Suggested lane |
| --- | --- | --- |
| E2E dashboard smoke | Proves the browser flow beyond build-time checks: open dashboard, load demo source, process frames, inspect counts/events/sessions/export. | normal |
| Demo runbook and seed data | Makes the project reproducible for reviewers without reverse-engineering commands or sample data. | normal |
